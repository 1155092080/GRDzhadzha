...
class SpatialDepScalarPotential
{
  ...
  public:
                           ...
                           const Coordinates<data_t> &coords) const
    {
        ...
        V_of_phi = (0.5 * mu * mu * vars.phi * vars.phi + 1.0/2.0/3.0/4.0 * lambda * vars.phi * vars.phi * vars.phi * vars.phi) * pow(1.0 + 1.0/pi*(atan(abs(r)-100.0)+pi/2.0), 2);

        // The potential gradient at phi wrt the field
        // m^2 phi
        //dVdphi = mu * mu * vars.phi + 1.0/2.0/3.0 * lambda * vars.phi * vars.phi * vars.phi;
        dVdphi = (mu * mu * vars.phi + 1.0/2.0/3.0 * lambda * vars.phi * vars.phi * vars.phi) * pow(1.0 + 1.0/pi*(atan(abs(r)-100.0)+pi/2.0), 2);
    }
};
