...
class MimickerBH
{
  public:
    ...
    struct params_t
    {
        double rs = 6;  // Radius to smooth the Newtonian potential to avoid singularity
        double mu = 1.0;   // Multiplier to modify the strength of Newtonian gravitational field
        double pi = M_PI;
                                   ...
                                   const Coordinates<data_t> &coords) const
    {
        ...
        const data_t H = simd_conditional(simd_compare_gt(r, rs), -mu*M/r, mu*2.0/3.0*pi*pow(r,2)*dens - mu*2.0/3.0*pi*pow(rs,2)*dens - mu*M/rs);
